...
사용법: DATABASE_URL 환경변수 설정 후
  python load_luris.py --law <법령정보.xlsx> --act <행위제한정보.xlsx> --month YYYYMM
"""
...
def parse_law(path):
        ...
        for kind, flag in (('bcr', is_bp), ('far', is_yr)):
            if flag:
                raw.append(dict(code=code, zone_cd=str(r[1]).strip(), zone_nm=str(r[2]).strip(),
                                kind=kind, jo=jo_m.group(1) if jo_m else '', prov=str(r[7]).strip(),
    ...
    out = []
    for x in raw:
        is_base = x['self_match'] and x['jo'] == base_jo.get((x['code'], x['kind']))
