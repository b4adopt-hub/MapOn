...
export default function AuthModal({ onClose, signIn, signUp }: Props) {
  ...
  async function submit() {
    ...
    if (mode === 'login') {
      ...
      if (e) setErr(e);
      else setMsg('가입 확인 메일을 보냈습니다. 메일의 링크를 눌러 인증을 완료하세요. (설정에 따라 바로 로그인될 수도 있습니다.)');
    }
    setBusy(false);
  }
    ...
    <div className="mp-modal-bg" onClick={onClose}>
      <div className="mp-modal" onClick={e => e.stopPropagation()}>
        <div className="mp-modal-tabs">
          <button className={mode === 'login' ? 'on' : ''} onClick={() => { setMode('login'); setErr(null); setMsg(null); }}>로그인</button>
          <button className={mode === 'signup' ? 'on' : ''} onClick={() => { setMode('signup'); setErr(null); setMsg(null); }}>회원가입</button>
        </div>
        ...
        {msg && <div className="mp-modal-msg">{msg}</div>}
