...
const FUNCTION_URL = (import.meta.env.VITE_SUPABASE_URL as string|undefined)
...
export default function App() {
  ...
  async function lookup(){
    ...
    setLooking(true); setLookupErr(null); setResult(null);
    try{
      let data:LandLookup;
      if(supabaseReady && supabase){
        ...
        const r=await fetch(FUNCTION_URL,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({address:address.trim()})});
    ...
    <div className="page">
      ...
      <section className="form">
        ...
        <div className="field">
          <label>목적</label>
          <div className="chips">
            {PURPOSES.map(p=>(<button key={p} className={`chip ${purpose===p?'active':''}`} onClick={()=>setPurpose(p)}>{PURPOSE_LABELS[p]}</button>))}
          </div>
        </div>
        <div className="field">
